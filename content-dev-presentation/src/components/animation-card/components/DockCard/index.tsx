...
export const DockCard = ({ children }: DockCardProps) => {
  ...
  useMousePosition(
    {
      onChange: ({ value }) => {
        ...
        if (dock.width > 0) {
          ...
          if (dock.hovered) {
            void size.start(transformedValue)
          }
  ...
  const handleClick = () => {
    if (!isAnimating.current) {
      ...
      void y.start(-initialWidth / 2, {
        loop: () => {
          if (3 === timesLooped.current++) {
            timeoutRef.current = setTimeout(() => {
              void opacity.start(0)
              y.set(0)
  ...
  const hoverChildrenBorderColors = [
    "red",
    "blue",
    "green",
    "yellow",
    ...
    <div className={styles['dock-card-container']}>
      <animated.button
        ...
        style={{
          ...
          height: size,
          y,
          borderColor: hoverChildrenBorderColors[hoverChildrenBorderColors.length * hoverChildrenBorderColors.length],
        }}
      >
        {children}
